...
- **Redundant Canvas Systems:** Product pages and Mystery Box pages are running up to 5 and 4 concurrent `<canvas>` elements respectively (e.g., `aknav-field`, `ak-product-field`, `ak-back-field`, `holo-rail-field`). This is a major source of GPU lag.
  ...
  - **Confirmed Cleanup (2026-04-05):** Collection-card image ratio changed from `"square"` (1:1) to `"portrait"` (4:5) on the default `collection.json` template. Reduces crop severity on non-square product images while preserving uniform card heights. Other collection templates unchanged.

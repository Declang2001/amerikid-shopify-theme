...
- **Redundant Canvas Systems:** Product pages and Mystery Box pages are running up to 5 and 4 concurrent `<canvas>` elements respectively (e.g., `aknav-field`, `ak-product-field`, `ak-back-field`, `holo-rail-field`). This is a major source of GPU lag.
  ...
  - **Combined Top-of-Page Prototype (2026-04-04):** `sections/ak-top.liquid` created — a single shared section replacing the two adjacent `custom_liquid` sections (holo-rail + aknav) with one canvas, one header bar, and one nav panel. Dust capped at 400–800, no dust shadowBlur, floater glow and full shooting star system preserved. Cart-only prototype: `templates/cart.json` migrated (removed ~81KB of inline custom_liquid). Remaining 17 templates unchanged pending visual verification. Visual continuity refinement applied same day: frosted-glass effect stripped from nav area so particles render continuously across full section height.

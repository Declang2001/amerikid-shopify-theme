...
- **Redundant Canvas Systems:** Product pages and Mystery Box pages are running up to 5 and 4 concurrent `<canvas>` elements respectively (e.g., `aknav-field`, `ak-product-field`, `ak-back-field`, `holo-rail-field`). This is a major source of GPU lag.
  ...
  - **Confirmed Cleanup (2026-04-05):** Product gallery settings restored. Changed `gallery_layout` from `"stacked"` to `"thumbnail"` and `image_zoom` from `"hover"` to `"lightbox"` in both `product.json` and `product.gp-template-bk-default.json`. Stacked layout was hiding thumbnails on desktop; hover zoom was hiding the lightbox button. Now shows clickable thumbnail grid and click-to-lightbox on product images.

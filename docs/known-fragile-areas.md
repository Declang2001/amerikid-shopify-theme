...
- **Multiple Canvases:** The interaction between multiple active canvases (`aknav-field`, `ak-product-field`, etc.) is likely sensitive to initialization timing.
  ...
  - **Note (2026-03-26):** `holo-rail-field` now uses the same cancel/restart pattern. Patched across all 18 JSON templates containing the holo-rail `custom_liquid` section (two code variants: 10 pretty-printed using `step`/`lastFrameTime`, 8 minified using `stp`/`lft`). Four of five known canvas systems are now patched. Remaining: `aknav-field`.

...
- **Multiple Canvases:** The interaction between multiple active canvases (`aknav-field`, `ak-product-field`, etc.) is likely sensitive to initialization timing.
  ...
  - **Note (2026-03-20):** `ak-back-field` now uses the same cancel/restart pattern. Its existing partial `IntersectionObserver` was upgraded from "skip draws but keep scheduling" to true cancellation. Three of five known canvas systems are now patched. Remaining: `aknav-field`, `holo-rail-field`.
